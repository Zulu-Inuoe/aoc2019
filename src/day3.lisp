(defpackage #:com.zulu.aoc2019.day3
  (:use
   #:alexandria
   #:cl)
  (:local-nicknames
...
(in-package #:com.zulu.aoc2019.day3)
...
(defun dist (p)
  "Manhattan distance of point `p'"
  (+ (abs (x p)) (abs (y p))))

(defun segments (path)
  "Convert a path of deltas into (p . s) pairs,
 where p is a point and s is delta to next point"
  (loop
    :for delta :in path
    :for p1 := #C(0.0 0.0) :then p2
    :for p2 := (+ delta p1)
    :collect (cons p1 delta)))

(defun intersection-point (l1 l2)
  "Find the intersection point or nil of line segments `l1' and `l2'"
  (let* ((p (car l1))
         (r (cdr l1))
         (q (car l2))
         (s (cdr l2)))
    (when (/= 0 (cross r s))
      (let ((tee (/ (cross (- q p) s) (cross r s)))
            (uuu (/ (cross (- p q) r) (cross s r))))
        (when (and (<= 0 tee 1)
                   (<= 0 uuu 1))
          (+ p (* tee r)))))))

(defun intersections (path1 path2)
  "Find all intersections between `path1' and `path2'"
  (let ((s1 (segments path1))
        (s2 (segments path2))
        (intersections ()))
    (dolist (l1 s1)
      (dolist (l2 s2)
        (when-let ((point (intersection-point l1 l2)))
          (push point intersections))))
    intersections))

(defun str->number (str)
  "Converts a string delta like U102 to a 2D coord"
  (check-type str string)
  (assert (> (length str) 1))
  (let ((value (parse-integer str :start 1)))
    (ecase (char str 0)
      (#\R (complex value 0))
      (#\L (complex (- value) 0))
      (#\U (complex 0 value))
